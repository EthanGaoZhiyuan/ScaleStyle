...
def main():
    ...
    if utility.has_collection(COLLECTION):
        ...
        schema = CollectionSchema(fields, description="ScaleStyle embeddings + scalar fields")
    ...
    for i in range(rounds):
        ...
        colours = chunk.get("colour_group_name", pd.Series([""] * len(chunk))).fillna("").astype(str).tolist()
        prices = chunk.get("price", pd.Series([0.0] * len(chunk))).fillna(0.0).astype(float).tolist()
