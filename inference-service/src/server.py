...
from src.utils.redis_client import validate_startup_connection
...
RAY_MEMORY_GB = float(os.getenv("RAY_MEMORY_GB", "3"))  # Total heap memory for Ray workers
RAY_OBJECT_STORE_GB = float(os.getenv("RAY_OBJECT_STORE_GB", "0.5"))  # Plasma shared memory

logger.info(f"Initializing Ray: memory={RAY_MEMORY_GB}GB, object_store={RAY_OBJECT_STORE_GB}GB")

validate_startup_connection()

ray.init(
    ignore_reinit_error=True,
    resources={"memory": int(RAY_MEMORY_GB * 1024 * 1024 * 1024)},  # Heap memory (Ray 2.x+ API)
    object_store_memory=int(RAY_OBJECT_STORE_GB * 1024 * 1024 * 1024),  # Plasma store (allocated at startup)
)
