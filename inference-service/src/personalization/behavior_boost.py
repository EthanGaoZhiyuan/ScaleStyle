...
class BehaviorBoost:
    ...
    def apply_boost(
        self, snapshot: PersonalizationSnapshot, results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        max_popularity = {
            "1h": max(
                (signals.get("1h", 0.0) for signals in popularity_signals.values()),
                default=0.0,
            ),
            "24h": max(
                (signals.get("24h", 0.0) for signals in popularity_signals.values()),
                default=0.0,
            ),
            "7d": max(
                (signals.get("7d", 0.0) for signals in popularity_signals.values()),
                default=0.0,
            ),
        }
        ...
        for result in results:
            ...
            if max_popularity["1h"] > 0:
                popularity_factor += self.popularity_1h_boost * (
                    math.log1p(popularity_signal.get("1h", 0.0))
                    / math.log1p(max_popularity["1h"])
                )
            if max_popularity["24h"] > 0:
                popularity_factor += self.popularity_24h_boost * (
                    math.log1p(popularity_signal.get("24h", 0.0))
                    / math.log1p(max_popularity["24h"])
                )
            if max_popularity["7d"] > 0:
                popularity_factor += self.popularity_7d_boost * (
                    math.log1p(popularity_signal.get("7d", 0.0))
                    / math.log1p(max_popularity["7d"])
                )
            ...
            if boost_factor > self.max_boost_cap:
                logger.debug(
                    f"Capping boost factor {boost_factor:.2f} -> {self.max_boost_cap:.2f} for item {article_id}"
                )
        ...
        avg_boost_factor = (
            (total_boost_factor / boosted_items) if boosted_items > 0 else 1.0
        )
