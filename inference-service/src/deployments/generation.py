...
@serve.deployment(ray_actor_options={"num_cpus": 0.05})
class GenerationDeployment:
    ...
    def __init__(self):
        ...
        - GENERATION_REVISION: Immutable HuggingFace model revision to load
        ...
        # ---- config ----
        self.model_name = GenerationConfig.MODEL
        self.model_revision = GenerationConfig.REVISION
        ...
        logger.info(
            f"Loading generation model: {self.model_name}@{self.model_revision} on device: {self.device}"
        )
        t0 = time.time()
