...
async def test_empty_candidates_fallback_popularity(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.ray_serve.deployments.ingress._redis_client",
        lambda: DummyRedis()
    )
    ...
    reranker = FakeHandle(score=lambda q, docs: {
        "scores": [0.1] * len(docs),
        "rerank_ms": 1,
        "mode": "stub"
    })
    ...
    resp = await ing.search(SearchRequest(query="red dress", k=2, debug=True, user_id="u2"))
    ...
    print(f" Empty retrieval fallback successful, returned {len(resp['results'])} results")
...
async def test_redis_timeout_graceful_degradation(monkeypatch):
    ...
    router = FakeHandle(route=lambda q, user_id=None: {"intent": "SEARCH", "filters": {}, "flow": "smart"})
    embed = FakeHandle(embed=lambda q, is_query=True: [0.1, 0.2, 0.3])
    retrieval = FakeHandle(search=lambda vector, **kw: [{"article_id": "1", "score": 0.9}])
...
async def test_ab_flow_base_no_rerank(monkeypatch):
    ...
    router = FakeHandle(route=lambda q, user_id=None: {"intent": "SEARCH", "filters": {}, "flow": "base"})
    embed = FakeHandle(embed=lambda q, is_query=True: [0.1, 0.2, 0.3])
    retrieval = FakeHandle(search=lambda vector, **kw: [{"article_id": "1", "score": 0.9}])
    popularity = FakeHandle(topk=lambda k: [{"article_id": "p1"}])
