...
class TestBehaviorBoostCap:
    ...
    def test_boost_below_cap_not_affected(self):
        ...
        booster = BehaviorBoost(
            exact_click_boost=1.5, max_boost_cap=3.0, debug_mode=False
        )

        snapshot = make_snapshot(recent_clicks=("item_456",))

        results = [{"article_id": "item_456", "rerank_score": 10.0}]
    ...
    def test_debug_mode_disabled_no_debug_info(self):
        ...
        booster = BehaviorBoost(
            exact_click_boost=1.5, debug_mode=False  # Debug mode OFF
        )

        snapshot = make_snapshot(recent_clicks=("item_789",))

        results = [{"article_id": "item_789", "rerank_score": 10.0}]
    ...
    def test_debug_mode_enabled_adds_debug_info(self):
        ...
        booster = BehaviorBoost(exact_click_boost=1.5, debug_mode=True)  # Debug mode ON

        snapshot = make_snapshot(recent_clicks=("item_999",))

        results = [{"article_id": "item_999", "rerank_score": 10.0}]
    ...
    def test_category_boost_with_cap(self):
        ...
        results = [{"article_id": "item_dress_123", "rerank_score": 10.0}]
    ...
    def test_update_config_updates_boost_cap(self):
        ...
        booster = BehaviorBoost(max_boost_cap=3.0)
    ...
    def test_behavior_boost_uses_only_snapshot_no_reader(self):
        ...
        snapshot = make_snapshot(
            ...
            popularity_signals={
                "item_123": {"1h": 0.0, "24h": 0.0, "7d": 0.0}
            },  # no popularity boost
        )
