...
def test_snapshot_loading_uses_bounded_pipelines_for_multiple_candidates():
    ...
    materialize_pipe = _pipeline_with_result([None, True, None, True, None, True, None, None, None])  # zunionstore + expire + delete for 3 windows
...
def test_snapshot_degrades_without_repeated_fetches_when_user_pipeline_fails():
    ...
    assert redis_client.pipeline.call_count == 5  # user + ttl + lock + materialize + item
...
def test_snapshot_degraded_log_uses_canonical_snake_case_fields(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        reader.load_personalization_snapshot("user-1", ["candidate_a"], max_recent_clicks=2)

    assert "user_id=user-1" in caplog.text
    assert "redis_round_trips=4" in caplog.text  # user failed, so ttl + lock + materialize + item
...
def test_repeated_snapshot_requests_reuse_local_materialized_windows_without_rebuilding():
    ...
    with patch("personalization.feature_reader.time.time", return_value=1000.0):
        first = reader.load_personalization_snapshot("user-1", ["candidate_a"], max_recent_clicks=1)
        second = reader.load_personalization_snapshot("user-1", ["candidate_a"], max_recent_clicks=1)
    ...
    assert redis_client.pipeline.call_count == 8  # 5 for first + cached-ttl pipe + user + item
...
def test_expired_materialization_rebuilds_once_then_reuses():
    ...
    materialize_pipe_1 = _pipeline_with_result([None, True, None, True, None, True, None, None, None])
    item_pipe_1 = _pipeline_with_result([
        "dress",
        [1.0],
        [2.0],
    ...
    materialize_pipe_2 = _pipeline_with_result([None, True, None, True, None, True, None, None, None])
    item_pipe_2 = _pipeline_with_result([
        "dress",
        [4.0],
        [5.0],
    ...
    redis_client.pipeline.side_effect = [
        user_pipe_1, ttl_pipe_1, lock_pipe_1, materialize_pipe_1, item_pipe_1,
        user_pipe_2, ttl_pipe_2, lock_pipe_2, materialize_pipe_2, item_pipe_2,
        user_pipe_3, cached_ttl_pipe, item_pipe_3,
    ]
    ...
    with patch("personalization.feature_reader.time.time", return_value=5001.0):
        third = reader.load_personalization_snapshot("user-1", ["candidate_a"], max_recent_clicks=1)

    assert first.redis_round_trips == 5  # user + ttl + lock + materialize + item
    assert second.redis_round_trips == 5  # user + ttl + lock + materialize + item (expired)
    assert third.redis_round_trips == 2  # user + item (cached)
    assert redis_client.pipeline.call_count == 13  # 5 + 5 + cached-ttl pipe + user + item
