...
################################################################################
# Integration Test Suite - Common Utilities
...
k8s_get_deployment_available_replicas() {
    local namespace="${1:-scalestyle}"
    local deployment="$2"
...
k8s_wait_for_deployment_replicas() {
    local namespace="${1:-scalestyle}"
    local deployment="$2"
    ...
    while [[ $elapsed -lt $timeout_seconds ]]; do
        local available
        available=$(k8s_get_deployment_available_replicas "$namespace" "$deployment")
        if [[ "$available" =~ ^[0-9]+$ ]] && [[ "$available" -ge "$expected_replicas" ]]; then
            return 0
        fi
...
export -f k8s_check_pod_ready k8s_get_pod_by_component k8s_list_pods
export -f k8s_get_deployment_replicas k8s_get_deployment_available_replicas
export -f k8s_wait_for_deployment_replicas k8s_delete_pod
